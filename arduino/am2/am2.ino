...
void fastSend(bool* packet, int packet_size, bool ack){
  noInterrupts();
  if (!ack){
    digitalWrite(AINETOUT, HIGH);
    delayMicroseconds(32);
    digitalWrite(AINETOUT, LOW);
...
void isr_read_msg(){
  ...
  while (digitalRead(AINETIN) == HIGH  && counts < BRAKE){
    counts++;
  }
...
void setup() {
  //init ainet pins
  pinMode(AINETIN, INPUT); 
  pinMode(AINETOUT, OUTPUT);
  //prepare uranus;
